...
export class CdkStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    dbInstance.connections.allowFromAnyIpv4(ec2.Port.tcp(5432), 'Allow App access to DB');

    // 5. Microservices (Fargate)
    const commonTaskProps = { memoryLimitMiB: 512, cpu: 256 };

    // IAM Service
    const iamService = new ecs_patterns.ApplicationLoadBalancedFargateService(this, 'IamService', {
      cluster,
      taskImageOptions: {
        image: ecs.ContainerImage.fromAsset(path.join(__dirname, '../../../services/iam-service')),
        environment: {
          PORT: '80',
    ...
    const productService = new ecs_patterns.ApplicationLoadBalancedFargateService(this, 'ProductService', {
      cluster,
      taskImageOptions: {
        image: ecs.ContainerImage.fromAsset(path.join(__dirname, '../../../services/product-service')),
        environment: {
          PORT: '80',
    ...
    // Inventory Service
    const inventoryService = new ecs_patterns.ApplicationLoadBalancedFargateService(this, 'InventoryService', {
      cluster,
      taskImageOptions: {
        image: ecs.ContainerImage.fromAsset(path.join(__dirname, '../../../services/inventory-service')),
        environment: {
          PORT: '80',
    ...
    const apiDestination = new events.ApiDestination(this, 'InventoryApiDestination', {
      connection: new events.Connection(this, 'InventoryConnection', {
        authorization: events.Authorization.apiKey('x-api-key', cdk.SecretValue.unsafePlainText('unused')), // Dummy for internal
      }),
    ...
    const iamIntegration = new apigateway.HttpIntegration(`http://${iamService.loadBalancer.loadBalancerDnsName}/`);
    const productIntegration = new apigateway.HttpIntegration(`http://${productService.loadBalancer.loadBalancerDnsName}/`);
    const orderIntegration = new apigateway.HttpIntegration(`http://${orderService.loadBalancer.loadBalancerDnsName}/`);
    const inventoryIntegration = new apigateway.HttpIntegration(`http://${inventoryService.loadBalancer.loadBalancerDnsName}/`);

    api.root.addResource('auth').addProxy({ defaultIntegration: iamIntegration });
    ...
    api.root.addResource('orders').addProxy({ defaultIntegration: orderIntegration });
    api.root.addResource('inventory').addProxy({ defaultIntegration: inventoryIntegration });
